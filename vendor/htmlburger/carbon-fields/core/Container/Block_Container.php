...
class Block_Container extends Container {
	...
	/**
	 * Set the default templates that should be rendered in inner blocks.
	 *
	...
	public function set_inner_blocks_template( $template = null ) {
		if ( ! is_array( $template ) && ! is_null( $template ) ) {
			throw new \Exception( __( "The templates must be an 'array' or 'null'.", 'crb' ) );
		}

		$this->settings[ 'inner_blocks' ][ 'templates' ] = $template;

		return $this;
	}

	/**
	 * Set the lock mode used by templates of inner blocks.
	 *
	...
	public function set_inner_blocks_template_lock( $lock = null ) {
		if ( is_string( $lock ) && ! in_array( $lock, [ 'all', 'insert' ] ) ) {
			throw new \Exception( __( "The templates lock must be 'all', 'insert', 'false' or 'null'.", 'crb' ) );
		}
